...
//! [`Api`](tagsyd::api::Api). The Dart UI holds one [`TagsyApp`] and never
//! learns which transport backs it.
//!
//! Two conventions make this layer usable from Dart, and both are load-bearing:
//! every id is a `String` (see the string-id section on [`TagsyApp`]), and
...
/// Dart *enum* the UI can pass to [`TagsyApp::run_query`]. Variants MUST
...
///
/// Variants MUST stay in sync with [`tagsyd::api::ApiError`]; see the
...
/// The result of [`TagsyApp::run_query`] as the flattened [`FileEntry`] /
...
impl TagsyApp {
    ...
    pub async fn attach() -> Result<TagsyApp, StartError> {
    ...
    /// — see [`Api::run_query`](tagsyd::api::Api::run_query)). This is what
    ...
    /// "no changes".
    ///
    /// The core [`tagsyd::api::EditOutcome`] type is flattened to a bare
...
pub enum OperationUpdateDto {
    ...
    /// [`TagsyApp::list_operations`] to re-sync its view.
    Resynced,
