...
use onisyncd::configuration::{Configuration, ConfigurationError};
...
fn run_thread(
    ...
    runtime.block_on(async move {
        let (api, driver) = match onisyncd::run(configuration, paths, shutdown).await {
            Ok(pair) => pair,
            Err(error) => {
        ...
        if let Err(error) = driver.await {
            log::error!("onisync runtime exited with error: {error}");
        }
    });
}
