/*
 * Foreground service scaffolding for the onisync Android app
 ...
 *   - the native runtime is owned by the Rust bridge (onisync-bridge crate):
 *     OniSyncApp.start(...) on service start, OniSyncApp.shutdown() on destroy.
 *
 * The actual OniSyncApp handle lives on the Dart side (via flutter_rust_bridge);
 * this service only guarantees the process stays alive. Depending on the app's
 * architecture you either:
 *   (a) keep the OniSyncApp handle in Dart and let this service just hold the
...
class OniSyncService : Service() {
    ...
    override fun onDestroy() {
        // The Rust runtime (OniSyncApp) is torn down from Dart; this service
    ...
    companion object {
        private const val CHANNEL_ID = "onisync_sync"
