...
/// `/tmp/tagsy.sock`, while an interactive CLI would look in
/// `/run/user/1000/tagsy.sock` and fail. One constant removes that whole
/// class of bug.
///
/// The directory `/run/tagsy` is owned and secured by systemd via
/// `RuntimeDirectory=tagsy` in the unit (see `module.nix`): it is created
