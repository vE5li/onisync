//! Thin desktop daemon wrapper around the `onisyncd` library.
//!
//! All runtime logic lives in the library (`onisyncd::run`); this
...
use onisyncd::ShutdownSignal;
use onisyncd::configuration::Configuration;
use onisyncd::control::serve_control;
use onisyncd::identity::Identity;
use onisyncd::paths::{Paths, control_socket_path};
...
enum Commands {
    /// Create this machine's long-lived identity key in `~/.onisync`.
    Keygen,
    Run {
...
fn paths_from_env() -> Paths {
    ...
    let identity_file = std::env::var("ONISYNC_PRIVATE_KEY_FILE")
        .expect("ONISYNC_PRIVATE_KEY_FILE environment variable not set");
    ...
    match arguments.command {
        ...
        Commands::Run { configuration_file } => {
            ...
            let (api, driver) = match onisyncd::run(configuration, paths, shutdown.clone()).await {
                Ok(pair) => pair,
                Err(error) => {
                    log::error!("onisync runtime failed to start: {error}");
            ...
            if let Err(error) = run_result {
                log::error!("onisync runtime failed: {error}");
                return Err(std::io::Error::other(error.to_string()));
            }
        }
    }
